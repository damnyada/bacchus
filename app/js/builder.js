...
function changeSlices(qtd) {
	qtd = qtd > 20 ? 20 : qtd;
	var mainSlices = getMainSlices();
	var sliceWrapper = document.getElementById('row-wrapper');
	var fields = '';

	if (qtd > mainSlices.length) {
		// add rows to HTML
		for (var i = mainSlices.length + 1; i <= qtd; i++) {
	...
	} else if (qtd < mainSlices.length) {
		// delete rows from HTML
		for (i = mainSlices.length - 1; i >= qtd; i--) {
